...
class KeyRotationTest(unittest.TestCase):
    ...
    def test_is_key_expiring(self, mock_datetime):
        mock_datetime.today.return_value = datetime.datetime(2019, 8, 1)
        mock_datetime.strptime.side_effect = [datetime.datetime(2019, 7, 29), datetime.datetime(2019, 6, 1)]
        expiring_key = {'validAfterTime': '2019-07-29T22:11:10Z'}
        valid_key = {'validAfterTime': '2019-06-01T22:11:10Z'}
        self.assertTrue(key_rotation.is_key_expiring(expiring_key))
        self.assertFalse(key_rotation.is_key_expiring(valid_key))
